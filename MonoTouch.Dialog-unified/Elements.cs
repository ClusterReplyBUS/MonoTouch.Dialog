...
namespace MonoTouch.Dialog
{
	...
	/// </summary>
	public partial class nfloatElement : Element {
		public bool ShowCaption;
		public nfloat Value;
		public nfloat MinValue, MaxValue;
		static NSString skey = new NSString ("nfloatElement");
		//UIImage Left, Right;
		UISlider slider;

		public nfloatElement (nfloat value) : this (null, null, value)
		{
		}
		
		public nfloatElement (UIImage left, UIImage right, nfloat value) : base (null)
		{
			//Left = left;
			//Right = right;
			MinValue = 0;
		...
		public override UITableViewCell GetCell (UITableView tv)
		{
			...
			if (slider == null){
				slider = new UISlider (new CGRect (10f + captionSize.Width, UIDevice.CurrentDevice.CheckSystemVersion (7, 0) ? 18f : 12f, cell.ContentView.Bounds.Width - 20 - captionSize.Width, 7f)) {
					BackgroundColor = UIColor.Clear,
					MinValue = (float)this.MinValue,
                    MaxValue = (float)this.MaxValue,
					Continuous = true,
	...
	internal static class Helper {
		...
		static public CGSize StringSize (this NSString self, UIFont font, nfloat width, UILineBreakMode mode)
		{
			// FIXME: use GetBoundingRect instead
			return self.GetSizeUsingAttributes (new UIStringAttributes () {
				Font = font,
		...
		static public CGSize DrawString (this string self, CGPoint point, nfloat width, UIFont font, UILineBreakMode mode)
		{
			// FIXME
	...
	public partial class StyledMultilineElement : StyledStringElement, IElementSizing {
		...
		{
			nfloat margin = UIDevice.CurrentDevice.UserInterfaceIdiom == UIUserInterfaceIdiom.Phone ? 40f : 110f;
			CGSize maxSize = new CGSize (tableView.Bounds.Width - margin, nfloat.MaxValue);
	...
	public partial class MultilineElement : StringElement, IElementSizing {
		...
		{
			nfloat margin = UIDevice.CurrentDevice.UserInterfaceIdiom == UIUserInterfaceIdiom.Phone ? 40f : 110f;
			CGSize size = new CGSize (tableView.Bounds.Width - margin, nfloat.MaxValue);
		...
		public override UITableViewCell GetCell (UITableView tv)
		{
			...
			using (var cs = CGColorSpace.CreateDeviceRGB ()){
				using (var bit = new CGBitmapContext (IntPtr.Zero, dimx, dimy, 8, 0, cs, CGImageAlphaInfo.PremultipliedFirst)){
					...
					if (roundBottom){
						bit.AddArc (rad, rad, rad, (nfloat) Math.PI, (nfloat) (3*Math.PI/2), false);
					} else {
						bit.MoveTo (0, rad);
					...
					if (roundTop){
						bit.AddArc (rad, dimy-rad, rad, (nfloat) (Math.PI/2), (nfloat) Math.PI, false);
	...
	public partial class DateTimeElement : StringElement {
		...
		public DateTimeElement (string caption, NSDate date) : base (caption)
		{
			DateValue = date;
			Value = FormatDate (date.ToDateTime());
		}	
		
		public override UITableViewCell GetCell (UITableView tv)
		{
			Value = FormatDate (DateValue.ToDateTime());
			var cell = base.GetCell (tv);
			cell.Accessory = UITableViewCellAccessory.DisclosureIndicator;
            cell.SelectionStyle = UITableViewCellSelectionStyle.Blue;
			return cell;
		...
		class MyViewController : UIViewController {
			...
			public override void ViewWillDisappear (bool animated)
			{
				...
				container.DateValue = (NSDate) container.datePicker.Date;
	...
	public partial class DateElement : DateTimeElement {
		public DateElement (string caption, NSDate date) : base (caption, date)
		{
        ...
        {
            //return fmt.ToString(GetDateWithKind(dt.ToDateTime()));
            return fmt.ToString(GetDateWithKind(dt).ToNSDate());
		}

	...
	public partial class MessageSummaryView : UIView {
		...
		static MessageSummaryView ()
		{
			using (var colorspace = CGColorSpace.CreateDeviceRGB ()){
				gradient = new CGGradient (colorspace, new nfloat [] { /* first */ .52f, .69f, .96f, 1, /* second */ .12f, .31f, .67f, 1 }, null); //new nfloat [] { 0, 1 });
			}
		}
		...
		public void Update (string sender, string body, string subject, NSDate date, bool newFlag, int messageCount)
		{
			Sender = sender;
			Body = body;
			Subject = subject;
		...
		public override void Draw (CGRect rect)
		{
            ...
            var diff = NSDate.Now.ToDateTime() - Date.ToDateTime();
			var now = DateTime.Now;
			string label;
            if (now.Day == Date.ToDateTime().Day && now.Month == Date.ToDateTime().Month && now.Year == Date.ToDateTime().Year)
                label = Date.ToDateTime().ToShortTimeString();
			else if (diff <= TimeSpan.FromHours (24))
				label = "Yesterday".GetText ();
			else if (diff < TimeSpan.FromDays (6))
                label = Date.ToDateTime().ToString("dddd");
			else
                label = Date.ToDateTime().ToShortDateString();
			...
			Sender.DrawString (new CGPoint (offset, 2), (nfloat)(bw-dateSize), SenderFont, UILineBreakMode.TailTruncation);
			Subject.DrawString (new CGPoint (offset, 23), (nfloat)(bw-offset-boxWidth), SubjectFont, UILineBreakMode.TailTruncation);

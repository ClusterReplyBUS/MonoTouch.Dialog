...
namespace MonoTouch.Dialog
{
	...
	public class RangeAttribute : Attribute {
		public RangeAttribute (nfloat low, nfloat high)
		{
			Low = low;
			High = high;
		}
		public nfloat Low, High;
	...
	public class BindingContext : IDisposable {
		...
		void Populate (object callbacks, object o, RootElement root)
		{
			...
			foreach (var mi in members){
				...
				if (mType == typeof (string)){
				...
				} else if (mType == typeof (nfloat)){
					var nfloatElement = new nfloatElement (null, null, (nfloat) GetValue (mi, o));
					nfloatElement.Caption = caption;
					element = nfloatElement;
					
					foreach (object attr in attrs){
						if (attr is RangeAttribute){
							var ra = attr as RangeAttribute;
							nfloatElement.MinValue = ra.Low;
							nfloatElement.MaxValue = ra.High;
							nfloatElement.ShowCaption = ra.ShowCaption;
						}
					}
				...
				} else if (mType == typeof (NSDate)){
					var NSDate = (NSDate) GetValue (mi, o);
					...
					if (asDate)
						element = new DateElement (caption, NSDate);
					else if (asTime)
						element = new TimeElement (caption, NSDate);
					else
						 element = new DateTimeElement (caption, NSDate);
		...
		public void Fetch ()
		{
			foreach (var dk in mappings){
				...
				else if (element is nfloatElement)
					SetValue (mi, obj, ((nfloatElement) element).Value);

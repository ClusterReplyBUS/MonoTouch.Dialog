...
namespace MonoTouch.Dialog
{
    ...
    public class DialogViewController : UITableViewController
    {
        ...
        public void PerformFilter(string text)
        {
            ...
            var newSections = new List<Section>();
            newSections.Add(new Section(originalSections[0].Header, originalSections[0].Footer)
            {
                FooterView = originalSections[0].FooterView,
                HeaderView = originalSections[0].HeaderView,
                Elements = originalSections[0].Elements.Where(x => x.Matches(text)).ToList(),

            });

            originalElements = new Element[originalSections[0].Elements.Count][];
            originalElements[0] = originalSections[0].Elements.ToArray();
        ...
        public virtual void Selected(NSIndexPath indexPath)
        {
            ...
            if (element is TaggedRadioElement)
            {
                if (originalElements != null)
                {
                    foreach (var original in originalElements)
                    {
                        ...
                        {
                            if (((StringElement)original[i]).Caption == ((StringElement)element).Caption)
                            {
                                element = original[i];
        ...
        public override void LoadView()
        {
            ...
            if ((root is TaggedRootElement<TaggedRadioElement>) || (root is TaggedRootElement<TaggedCheckboxElement>))
                EnableSearch = true;
            else
                EnableSearch = false;

...
namespace MonoTouch.Dialog
{
    ...
    public partial class Element : IElementSizing, IDisposable
    {
        ...
        public virtual nfloat GetHeight(UITableView tableView, NSIndexPath indexPath)
        {
            ...
            if (_cell == null)
            {
                _cell = GetCell(tableView);
            }
            UIEdgeInsets margins = _cell.LayoutMargins;

            string reference = (Caption ?? string.Empty).Length > (_cell.TextLabel.Text ?? string.Empty).Length ? Caption : _cell.TextLabel.Text;
            int count = 0;
            int captionLenght = 0;
            if (reference != null)
            {
            ...
            numLines = (int)Math.Ceiling(captionLenght / width) + count;
            //cell.TextLabel.Lines = numLines;
            int lineCount = 0;
            if (_cell != null && _cell.DetailTextLabel != null && _cell.DetailTextLabel.Text != null)
            {
                lineCount = (int)_cell.DetailTextLabel.Lines;
                using (StringReader r = new StringReader(_cell.DetailTextLabel.Text))
                {
            ...
            return Math.Max(50, (numLines * height));
        }
    }
    ...
    public partial class RadioElement : StringElement
    {
        ...
        public override void Selected(DialogViewController dvc, UITableView tableView, NSIndexPath indexPath)
        {
            ...
            if (RadioIdx != indexPath.Row)
            {
                UITableViewCell cell;
                var selectedIndex = root.PathForRadio(indexPath.Row);
               ...
               // root.RadioSelected = RadioIdx;
                root.RadioSelected = indexPath.Row;
            }
            else
                root.RadioSelected = indexPath.Row;
